...
def _is_empty_elem(elem: ET.Element) -> bool:
    """判断元素是否为空元素（无子元素、无有效文本，即 self-closing 式）。"""
    return len(elem) == 0 and not (elem.text and elem.text.strip())


def check_mixed_content(filepath: str) -> list[str]:
    """
    检查混合内容：
    - 元素若含有效文本节点，则不允许有非空（非 self-closing 式）子元素；
    - 反之，元素若含非空子元素，则不允许再有有效文本节点。
    返回错误信息列表（空列表表示通过）。
    """
    errors = []
    try:
        ...
        return errors  # 格式错误已由 well-formed 检查报告
    ...
    def walk(elem: ET.Element, path: str):
        ...
        container_children = [c for c in elem if not _is_empty_elem(c)]
        if has_text and container_children:
            tags = ", ".join(f"<{c.tag}>" for c in container_children)
            errors.append(f"{path}: 文本节点与非空子元素混合 ({tags})")
        for i, child in enumerate(elem):
            walk(child, f"{path}/{child.tag}[{i + 1}]")
...
def main():
    ...
    for filepath in xml_files:
        ...
        if ok:
            mixed_errors = check_mixed_content(filepath)
            if mixed_errors:
                print(f"  ✗ {relpath}")
                for err in mixed_errors:
                    print(f"    └─ {err}")
                failed += 1
                failed_files.append((relpath, "; ".join(mixed_errors)))
            else:
                print(f"  ✓ {relpath}")
                passed += 1
        else:
